/**
 * Merge engine for @glance-apps/sync.
 *
 * Instead of "last write wins" at the whole-dataset level, these functions
 * merge arrays by ID — keeping newer versions when both sides have the same
 * item, preserving items that only exist on one side, and respecting
 * tombstones for permanently deleted items.
 */
...
/**
 * Generic array merge by item ID, preserving local ordering and appending
 * remote-only items at the end.
 *
 * @param {Array}  localItems    - Items from this device
 * @param {Array}  remoteItems   - Items from the server
 * @param {Object} deletedIds    - Map of item ID → deletion timestamp (tombstones)
 * @param {Date|null} [syncHorizon] - Items older than this date that are local-only
 *   are presumed zombie (tombstone pruned on remote) and silently dropped.
 * @param {Object} [options]
 * @param {string} [options.idField='id']               - Field name used as the item identifier
 * @param {string} [options.timestampField='updatedAt'] - Field name used for last-modified comparison
 * @returns {{ merged: Array, localChanged: boolean, remoteChanged: boolean }}
 */
export const mergeArrayById = (localItems, remoteItems, deletedIds, syncHorizon = null, { idField = 'id', timestampField = 'updatedAt' } = {}) => {
  const remoteMap = new Map(remoteItems.map(t => [String(t[idField]), t]));
  const localIds = new Set(localItems.map(t => String(t[idField])));
  let localChanged = false;
  let remoteChanged = false;
  const merged = [];

  // First pass: iterate local items in order
  for (const localItem of localItems) {
    const id = String(localItem[idField]);
    if (deletedIds[id] && new Date(deletedIds[id]) > new Date(localItem[timestampField] || 0)) {
      localChanged = true; // Removed a local item
      continue;
    }
    const remoteItem = remoteMap.get(id);
    if (remoteItem) {
      const localTime = new Date(localItem[timestampField] || 0);
      const remoteTime = new Date(remoteItem[timestampField] || 0);
      if (remoteTime > localTime) {
        ...
        merged.push(localItem); // Equal timestamps — keep local
      }
    } else {
      // Only in local. If the item is older than the sync horizon, the remote's
      // tombstone for it was likely pruned — treat it as a zombie rather than
      // uploading it and resurrecting a deleted item.
      if (syncHorizon && localItem[timestampField] && new Date(localItem[timestampField]) < syncHorizon) {
  ...
  // Second pass: append remote-only items
  for (const remoteItem of remoteItems) {
    const id = String(remoteItem[idField]);
    if (localIds.has(id)) continue;
    if (deletedIds[id] && new Date(deletedIds[id]) > new Date(remoteItem[timestampField] || 0)) {
...
export const pruneTombstones = (tombstones, cutoff) => {
...
export const mergeSyncData = (localData, remoteData, retentionDays = 90) => {
  ...
  // Merge each task list (tasks use 'lastModified' as their timestamp field)
  const taskOpts = { idField: 'id', timestampField: 'lastModified' };
  const tasksMerge = mergeArrayById(localData.tasks || [], remoteData.tasks || [], allDeletedIds, syncHorizon, taskOpts);
  const unschedMerge = mergeArrayById(localData.unscheduledTasks || [], remoteData.unscheduledTasks || [], allDeletedIds, syncHorizon, taskOpts);
  const binMerge = mergeArrayById(localData.recycleBin || [], remoteData.recycleBin || [], allDeletedIds, syncHorizon, taskOpts);
  const recurMerge = mergeArrayById(localData.recurringTasks || [], remoteData.recurringTasks || [], allDeletedIds, syncHorizon, taskOpts);
  ...
  if (localRoutinesDate === remoteRoutinesDate) {
    todayRoutinesMerge = mergeArrayById(localData.todayRoutines || [], remoteData.todayRoutines || [], todayRoutineTombstones, syncHorizon, taskOpts);
